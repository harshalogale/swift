...
struct ArcView: View {
    ...
    var body: some View {
        ...
        return ZStack() {
            ...
            arcShape.stroke(Color.primary, lineWidth: ArcView.isNodeSelected(arc, configuration: configuration) ? 4 : 0).clipShape(arcShape).animation(animation)
        ...
        }.contentShape(arcShape).onTapGesture {
            guard self.configuration.allowsSelection else { return }
            if self.configuration.selectedNode == self.arc.node && self.configuration.focusedNode == self.arc.node {
                self.configuration.focusedNode = self.configuration.parentForNode(self.arc.node)
            } else if self.configuration.selectedNode == self.arc.node {
                self.configuration.focusedNode = self.arc.node
            } else {
                self.configuration.selectedNode = self.arc.node
            }
        }
    }

    static func isNodeSelected(_ arc: Sunburst.Arc, configuration: SunburstConfiguration) -> Bool {
...
struct ArcLabel: View {
    ...
    var body: some View {
        VStack() {
            ...
            if !arc.isTextHidden {
                Text(LocalizedStringKey(arc.node.name))
                    .font(ArcView.isNodeSelected(self.arc, configuration: self.configuration) ? .headline: .body)
                    .fontWeight(ArcView.isNodeSelected(self.arc, configuration: self.configuration) ? .heavy: .none)
                    .animation(.none)
            }
        }
